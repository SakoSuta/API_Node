...
const PromoCodeController = {
  getAllPromoCodes: async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ error: 'An error occurred while retrieving promo codes.' });
    }
  },
  ...
  getPromoCodeByUuid: async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ error: 'An error occurred while retrieving the promo code.' });
    }
  ...
  createPromoCode: async (req, res) => {
    ...
    try {
      ...
      const uuid = uuidv4();
      const promoCode = await prisma.promoCode.create({ data: { name, code, discount, startDate, endDate, uuid } });
      res.json({promoCode, message: 'Promo code successfully created.'});
    }catch (error){
      console.error(error);
      if(error.code === 'P2002' && error.meta?.target?.includes('name')){
        res.status(400).json({ error: 'This name is already used by another promo code.' });
      }else if(error.code === 'P2002' && error.meta?.target?.includes('code')){
        res.status(400).json({ error: 'This code is already used by another promo code.' });
      }
      res.status(500).json({ error: 'An error occurred while creating the promo code.' });
    }
  },
  ...
  updatePromoCode: async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: 'An error occurred while updating the promo code.' });
    }
  },
  ...
  deletePromoCode: async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ error: 'An error occurred while deleting the promo code.' });
    }
  },
};
